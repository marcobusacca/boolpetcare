<section>
    <header>
        ...
        <p class="mt-1 text-muted">
            {{ __("Aggiorna le tue informazioni personali e l'email.") }}
        </p>
    </header>
            ...
            <div>
                <p class="text-sm mt-2 text-muted">
                    ...
                    <button form="send-verification" class="btn btn-outline-dark">
                        {{ __('Clicca qui per rinviare l\'email di conferma.') }}
                    </button>
                </p>

                @if (session('status') === 'verification-link-sent')
                <p class="mt-2 text-success">
                    {{ __('Un nuovo link di verifica è stato inviato al tuo indirizzo di posta elettronica.') }}
                </p>
                @endif
            </div>
            @endif
